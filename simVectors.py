...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.numSecondsToHoldEmotion = 30

        #TODO: get speed from time to show emotion
        self.speed = 2
        self.timeToShowEmotion = 100
...
def main():
    ...
    windowEmotions = [window.joy, window.sadness, window.anger, window.disgust, window.fear, window.surprise]
    userInputEmotion = None
    userInputPercentange = None
    while userInputEmotion != 'q' and userInputPercentange != 'q':
        userInputEmotion = input("Enter emotion, type 'q' to quit:")
        if userInputEmotion == 'q':
            continue
        if userInputEmotion not in emotions:
            print("Invalid emotion. Please enter a valid emotion from the list:", emotions)
            continue
        userInputPercentange = input("Enter percentage, type 'q' to quit:")
        # Validate and convert percentage input
        try:
            percentage = float(userInputPercentange)
        except ValueError:
            print("Invalid percentage. Please enter a valid number.")
            continue

        # Set the current emotion and percentage based on the user input

        # Set the current emotion and percentage based on the emotion name
        for i in range(len(emotions)):
            if emotions[i] == userInputEmotion:
                window.currEmotion = windowEmotions[i]
                window.currPercentOfEmotion = float(userInputPercentange)

        #reset timeElapsed back to zero each time we get successful input
        #reset direction to 2 each time we get successful input
        ...
        blinkTimeRange = (5,30) #if changing blinkTimeRange need to also change blinktime in faceAnimation.py 
